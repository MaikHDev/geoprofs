    ...
    const LogDetails = ({item}: { item: LogItem }) => {
        ...
        return (
            <div className="border p-4 rounded-md space-y-2 bg-gray-50">
                ...
                <div className="grid grid-cols-3 gap-4 font-medium border-b pb-1">
                    <div>Field</div>
    ...
    return (
        <div className="p-6 space-y-6">
            <div className="flex items-center gap-2 flex-wrap mb-6">
                <BreadcrumbPill label="Contexts" onClick={() => handleView("contexts")}
                                active={logView === "contexts"}/>
                {context && (
                    <>
                        <span className="text-gray-400 font-bold">→</span>
                        <BreadcrumbPill label={context} onClick={() => handleView("events")}
                ...
                {events && (
                    <>
                        <span className="text-gray-400 font-bold">→</span>
                        <BreadcrumbPill label={events} onClick={() => handleView("logs")} active={logView === "logs"}/>
                    </>
                )}
                {selectedLog && (
                    <>
            ...
            {logView === "events" && (
                <div className="grid gap-3">
                    {LE.enumValues.map((evt) => {
                        ...
                        if ((evt !== 'logged_in' && context !== 'users') || context === 'users') {
                            return (
                                <CardButton
                                    key={evt}
                                    title={evt}
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-3 gap-4 mb-4">
                        <input type="datetime-local" value={startDate} onChange={(e) => setStartDate(e.target.value)}
                               className="border rounded p-2" placeholder="Start Date"/>
                        <input type="datetime-local" value={endDate} min={startDate}
                               onChange={(e) => setEndDate(e.target.value)}
                               className="border rounded p-2" placeholder="End Date"/>
                        <input type="text" value={selectedUser} onChange={(e) => setSelectedUser(e.target.value)}
                               className="border rounded p-2" placeholder="User"/>
                    </div>
            ...
            {logView === "single" && selectedLog && (
                <div className="space-y-4">
                    <div className="border p-4 rounded-md bg-gray-50 space-y-2">
                        <div><span
                            className="font-semibold">Name: </span>{selectedLog.user?.name} {selectedLog.user?.lastName}
                        </div>
                        <div><span className="font-semibold">Email: </span>{selectedLog.user?.email}</div>
                        <div><span className="font-semibold">Role: </span>{selectedLog.user?.role ?? "-"}</div>
                        <div><span
                            className="font-semibold">Date: </span>{new Date(selectedLog.createdAt).toLocaleString()}
                        </div>
                        <div><span className="font-semibold">Event: </span>{selectedLog.logEvent}</div>
                        <div><span className="font-semibold">Context: </span>{selectedLog.logContext}</div>
                    </div>
